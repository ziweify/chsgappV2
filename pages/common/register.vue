<template>
  <view class="login-container">
    <view class="logo-box toll" :style="{'paddingTop':50+winobj.statusBarHeight+'px'}">
      <image src="/static/img/mlogo.png"></image>
    </view>
    <view class="content">
      <view class="form">
        <view class="input-box-wrap">
          <view class="input-bg">
            <input v-model="register.nickname" type="text" placeholder="请输入昵称,留空自动生成" class="username">
          </view>
          <view class="input-bg">
            <input v-model="register.account" type="text" placeholder="请输入6-16位字母和数字组合的账号" class="username">
          </view>
          <view class="input-bg" style="display: flex;">
            <input v-model="register.password" type="text" placeholder="请输入8-16位的密码" class="username" :password="eyename == 'eye'"
                   autocomplete="new-password">
            <u-icon style="margin-right: 14rpx;height: 42px;" color="#31C9E8" size="22" v-show="eyename == 'eye'" :name="eyename" @click="clearInput('eye-fill')"></u-icon>
            <u-icon style="margin-right: 14rpx;height: 42px;" color="#31C9E8" size="22" v-show="eyename == 'eye-fill'" :name="eyename" @click="clearInput('eye')"></u-icon>
          </view>
          <view class="input-bg password" style="display: flex;">
            <input v-model="register.repassword" type="text" placeholder="请再次填写密码" class="username" :password="eyename1 == 'eye'"
                   autocomplete="new-password">
            <u-icon style="margin-right: 14rpx;height: 42px;" color="#31C9E8" size="22" v-show="eyename1 == 'eye'" :name="eyename1" @click="clearInput1('eye-fill')"></u-icon>
            <u-icon style="margin-right: 14rpx;height: 42px;" color="#31C9E8" size="22" v-show="eyename1 == 'eye-fill'" :name="eyename1" @click="clearInput1('eye')"></u-icon>
          </view>
          <view class="input-yzm">
            <input v-model="register.code" type="number" placeholder="请输入验证码" class="input-box">
            <view class="image-wrap">
              <image @tap="captcha()" :src="base64"></image>
            </view>
          </view>
        </view>

        <view class="submit-btn register" @click="doregister"><button></button></view>
        <view class="submit-btn to_login" @click="loginTo()"><button></button></view>
      </view>
    </view>
  </view>
</template>

<script>
export default {
  data() {
    return {
      hidepassword: true,
      eyeclass: "tn-icon-eye",
      hidepassword1: true,
      eyeclass1: "tn-icon-eye",
      register: {
        account: '',
        password: '',
        repassword:'',
        nickname:'',
        code:'',
        key:'',
        logintype:0
      },
      base64: '',
      eyename: 'eye',
      eyename1: 'eye',
    };
  },
  computed:{
    winobj(){
      return this.$u.sys();
    }
  },
  onLoad(){
    this.captcha();
    //判断平台类型
    if (this.$u.os() === 'android' || this.$u.os() === 'ios') {
      this.register.logintype = 1;
    } else if (this.$u.os() === 'h5') {
      this.register.logintype = 2;
    }
  },
  methods: {
    clearInput(en) {
      this.eyename = en;
    },
    clearInput1(en) {
      this.eyename1 = en;
    },
    loginTo() {
      uni.$utils.jump("pages/common/login");
    },
    captcha() {
      let that = this;
      this.$u.api.common.getcaptcha().then(res => {
        that.register.key = res.data.key;
        //base64图片赋值
        that.base64 = 'data:image/png;base64,' + res.data.base64;
      }).catch(err => {
        uni.$utils.showToast("验证码获取失败");
      });
    },
    doregister(){
      let that = this;
      this.$u.throttle(async () => {
        const accountPattern = /^[a-zA-Z0-9]{6,16}$/;
        if (!accountPattern.test(this.register.account)) {
          uni.$utils.showToast('登陆账号必须是6-16位数字和字母组合');
          return;
        }
        const passwordPattern = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[A-Za-z\d]{8,16}$/;
        if (!passwordPattern.test(this.register.password)) {
          uni.$utils.showToast('密码需要8-16位数字和字母组合，要求至少有一个大写字母');
          return;
        }
        //登陆密码和确认密码是否一致
        if(this.register.password !== this.register.repassword){
          uni.$utils.showToast('两次密码输入不一致');
          return;
        }
        uni.showLoading({title: '注册中'});
        this.$u.api.member.register(this.register).then(res => {
          //console.log(res);
          uni.hideLoading();
          uni.setStorageSync('token', res.data.token);
          uni.setStorageSync('userInfo', res.data.userInfo);
          uni.setStorageSync('expiresTime', res.data.expires_time);
          
          // 重置WebSocket退出状态，允许重新连接
          /* if (uni.$socketUtils && uni.$socketUtils.resetExitStatus) {
            uni.$socketUtils.resetExitStatus();
            console.log('注册成功，已重置WebSocket退出状态');
          } */
          
          //等待存储成功后跳转
          uni.reLaunch({url: '/pages/home/home'});
        }).catch(res => {
          uni.hideLoading();
          that.$u.toast(res.msg);
          that.captcha();
        });
      },1000);
    }
  }
}
</script>

<style lang="scss" scoped>
.login-container {
  width: 100%;
  height: 100%;
  background: url(../../static/img/bg.jpg) no-repeat;
  background-size: 100% 100%;
  font-size: 14px;
  display: flex;
  justify-content: space-between;
  flex-direction: column;
}

.content {
  flex: 1;
  margin: 0 15px;
}

.logo-box {
  min-height: 60px;
  display: flex;
  justify-content: center;
  align-items: center;
}

.logo-box.toll image {
  width: 132px;
  height: 60px;
  display: block;
}

.form {
  background: url(../../static/img/register_bg.61c4151d.png);
  background-size: 100% 100%;
  background-repeat: no-repeat;
  margin-top: 30px;
  padding-bottom: 20px;
}

.input-box-wrap {
  padding-top:40px;
}

.input-bg {
  margin: 15px 20px;
  box-sizing: border-box;
  background: url(data:image/png;base64,) no-repeat;
  background-size: 100% 100%;
  overflow: hidden;
  position: relative;
  height: 42px;
}

.input-bg:first-child:before {
  content: "";
  position: absolute;
  display: inline-block;
  width: 18px;
  height: 20px;
  top: 11px;
  left: 18px;
  background: url(data:image/png;base64,) no-repeat;
  background-size: 100% 100%
}

.input-bg:nth-child(2):before, .input-bg:nth-child(3):before, .input-bg:nth-child(4):before {
  content: "";
  position: absolute;
  width: 16px;
  height: 19px;
  top: 10px;
  left: 18px;
  display: inline-block;
  background: url(data:image/png;base64,) no-repeat;
  background-size: 100% 100%
}

.input-bg:nth-child(2):before {
  background: url(data:image/png;base64,) no-repeat;
  background-size: 100% 100%
}

.input-bg input {
  width: 100%;
  background: none;
  border: none;
  box-sizing: border-box;
  height: 42px;
  padding-left: 45px;
  font-size: 14px;
}

.check-box-wrap {
  text-align: right;
  padding-right: 20px;
}

.submit-btn {
  padding: 0 20px;
  margin-top: 10px;
}

.submit-btn button {
  width: 100%;
  height: 44px;
  border: none
}

.submit-btn.register button {
  background: url(../../static/img/mem_register.f5f46261.png) no-repeat;
  background-size: 100% 100%
}

.submit-btn.to_login button {
  background: url(../../static/img/to_login.36702b4b.png) no-repeat;
  background-size: 100% 100%
}

.input-yzm {
  display: flex;
  align-items: center;
  justify-content: space-between;
  margin: 15px 20px;
  height: 42px;
  box-sizing: border-box;
  position: relative
}

.input-yzm input {
  border: none;
  flex: 1;
  height: 42px;
  border-radius: 24px;
  padding-left: 50px;
  font-size: 14px;
  line-height: 42px;
  background-color: #dae6f1;
  margin-right: 10px;
}

.input-yzm:before {
  content: "";
  position: absolute;
  width: 18px;
  height: 20px;
  top: 11px;
  left: 18px;
  display: inline-block;
  background: url(data:image/png;base64,) no-repeat;
  background-size: 100% 100%
}

.input-yzm .image-wrap {
  position: relative;
  overflow: hidden;
  border-radius: 4px;
  width: 200rpx;
  height: 38px;
}

.input-yzm image {
  max-width: 100%;
  height: 100%;
}

</style>
