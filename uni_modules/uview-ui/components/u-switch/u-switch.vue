<template>
	<view
	    ...
	    @tap="clickHandler"
	>
    <text :style="{'fontSize':(this.size-2)+'rpx'}" v-show="isActive && enableText" style="color: #fff;position: absolute;left: 6px;">{{ activeText }}</text>
		<view
		    class="u-switch__bg"
		    :style="[bgStyle]"
		>
		</view>
    <text :style="{'fontSize':(this.size-2)+'rpx'}" v-show="!isActive && enableText" style="color: #fff;margin-left: 4rpx;position: absolute;right: 6px;">{{ inactiveText }}</text>
		<view
	...
	export default {
		...
		computed: {
			...
			switchStyle() {
				...
				style.width = uni.$u.addUnit(this.size * 2 + 7)
			...
			nodeStyle() {
				...
				const translateX = this.isActive ? uni.$u.addUnit(this.space) : uni.$u.addUnit(this.size*1+5);
