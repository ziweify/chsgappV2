<template>
  <view class="layout">
    <z-paging ref="paging" v-model="list" :show-empty-view-reload="true" safe-area-inset-bottom @query="queryList" :default-page-size="20">
      <template #top>
        <TsCustom :backUrl="backUrl" :isBack="true" title="入群申请">
          <block slot='right'>
        ...
        <view class="list-h" style="margin-top: 15rpx;">
              ...
              <view class="span" style="flex:1;">申请时间</view>
              <view class="span" style="flex:1;">审核时间</view>
      ...
      <view class="list">
        <view v-for="(item,index) in list" :key="index" class="list-p">
          ...
          <view class="list-p-item" style="flex:1;color: rgb(156, 156, 156);font-size: 26rpx;">
            <view>{{ item.reviewTime1 }}</view> <view>{{ item.reviewTime2 }}</view>
          </view>
          <view class="list-p-item" style="flex:1;">
...
export default {
  ...
  data() {
    return {
      ...
      citem:[],
      cstatus:0,
      tabsList:[{title:'全部',key:'-1'},{title:'待审核',key:'1'},{title:'通过',key:'3'},{title:'拒绝',key:'2'},{title:'忽略',key:'4'}],
      dialogTitle:'',
    };
  },
  onLoad() {
  ...
  methods: {
    ...
    actionClicShow(item,type){
      ...
      this.dialogTitle = '是否'+(type == 3 ? '通过' : type == 2 ? '拒绝' : '忽略')+item.username+'的'+'入群申请?';
      this.citem = item;
      this.cstatus = type;
    },
    search(){
